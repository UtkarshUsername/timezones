...
export function App() {
  ...
  return <main className="min-h-screen overflow-x-hidden bg-[#e7edf5] px-4 py-6 text-slate-950 sm:px-8 sm:py-10"><div className="mx-auto max-w-6xl">
    <header className="mb-6 border-b-2 border-slate-950 pb-5"><h1 className="font-mono text-3xl font-bold tracking-tight sm:text-4xl">Timezones</h1></header>
    <section className="mb-7 flex flex-wrap items-center justify-between gap-4 border-y border-slate-400 py-4"><div className="flex flex-wrap items-center gap-5"><input aria-label="Date" className="border-b-2 border-slate-950 bg-transparent py-1 font-mono text-sm outline-none" type="date" value={planner.date} onInput={(event) => update({ date: event.currentTarget.value })} /><p className="border-l-2 border-orange-500 pl-4 font-mono text-sm"><b>{planner.start}–{planner.end}</b> · {shortZone(planner.sourceZone)}<br /><span className="text-xs text-slate-500">{zoneMeta(planner.date, planner.sourceZone)} · {sourceDate}</span></p></div><form className="flex gap-2" onSubmit={addZone}><select aria-label="Timezone to add" className="min-w-0 border-b-2 border-slate-950 bg-transparent px-1 py-2 text-sm outline-none" value={zoneInput} onChange={(event) => setZoneInput(event.currentTarget.value)}><option value="">Add zone</option>{zoneOptions.map((zone) => <option disabled={planner.zones.includes(zone)} key={zone} value={zone}>{shortZone(zone)} · {zoneMeta(planner.date, zone)}</option>)}</select><button aria-label="Add timezone" className="bg-slate-950 px-4 py-2 text-sm font-bold text-[#f9f7f1] transition hover:bg-orange-700" type="submit">Add</button></form></section>
    <section className="space-y-4">{planner.zones.map((zone) => <article className="grid gap-4 border-t border-slate-400 py-5 sm:grid-cols-[180px_1fr] sm:gap-7" key={zone} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); const movedZone = event.dataTransfer?.getData("text/plain") || draggedZone; const bounds = event.currentTarget.getBoundingClientRect(); const position = event.clientY < bounds.top + bounds.height * 0.35 ? "before" : "after"; if (movedZone) moveZone(movedZone, zone, position); setDraggedZone(null); }}><div className="flex justify-between gap-3 sm:block"><div className="flex items-start gap-2"><button aria-label={`Drag ${zone} to reorder`} className="mt-1 cursor-grab touch-none text-lg leading-none text-slate-400 hover:text-orange-700 active:cursor-grabbing" draggable onDragEnd={() => setDraggedZone(null)} onDragStart={(event) => { event.dataTransfer?.setData("text/plain", zone); setDraggedZone(zone); }} type="button">↕</button><div><p className="font-mono text-xl font-bold leading-none">{shortZone(zone)}</p><p className="mt-1 font-mono text-[11px] text-slate-500">{zone} · {zoneMeta(planner.date, zone)}</p><p className="mt-4 font-mono text-sm font-bold">{formatInZone(selectedRange.start, zone, { hour: "2-digit", minute: "2-digit", hourCycle: "h23" })}</p></div></div>{planner.zones.length > 1 ? <button aria-label={`Remove ${zone}`} className="h-fit text-xs font-bold uppercase tracking-wider text-slate-500 hover:text-orange-700" onClick={() => removeZone(zone)} type="button">Remove</button> : null}</div><Timeline date={planner.date} onSelect={selectRange} selectedEnd={selectedRange.end} selectedStart={selectedRange.start} zone={zone} /></article>)}</section>
  </div></main>;
}
