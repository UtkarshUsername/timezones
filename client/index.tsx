...
function Timeline({ zone, date, selectedStart, selectedEnd, onSelect }: { zone: string; date: string; selectedStart: number; selectedEnd: number; onSelect: (zone: string, start: string, end: string) => void }) {
  ...
  return <div className="relative"><div className="grid grid-cols-24 cursor-crosshair touch-none overflow-hidden rounded-sm border border-stone-300 bg-stone-100 focus:outline-none focus:ring-2 focus:ring-orange-600" onPointerDown={(event) => setDragStart(minuteAt(event))} onPointerUp={finish} role="slider" aria-label={`Select a time range in ${shortZone(zone)}`} tabIndex={0}>{cells.map(({ hour, active }) => <div aria-hidden="true" className={`h-14 border-r border-stone-300 last:border-r-0 transition-colors ${active ? "bg-orange-500" : hour < 7 || hour > 20 ? "bg-slate-800" : "bg-stone-100"}`} key={hour} />)}</div><div className="grid grid-cols-6 pt-1 text-[10px] font-semibold tracking-[0.12em] text-slate-500">{["00", "04", "08", "12", "16", "20"].map((hour) => <span key={hour}>{hour}</span>)}</div></div>;
}
