...
function Timeline({ zone, date, selectedStart, selectedEnd, onSelect, theme }: { zone: string; date: string; selectedStart: number; selectedEnd: number; onSelect: (zone: string, start: string, end: string) => void; theme: Theme }) {
  ...
  return <div className="relative"><div className={`grid grid-cols-24 cursor-crosshair touch-none overflow-hidden rounded-sm border focus:outline-none focus:ring-2 focus:ring-sky-400 ${dark ? "border-slate-700 bg-[#1e293b]" : "border-slate-200 bg-[#f8fafc]"}`} onPointerDown={(event) => { const start = minuteAt(event); setDragStart(start); onSelect(zone, timeFromMinutes(start), timeFromMinutes(start + 60)); }} onPointerMove={updateSelection} onPointerUp={finish} onPointerCancel={() => setDragStart(null)} role="slider" aria-label={`Select a time range in ${shortZone(zone)}`} tabIndex={0}>{cells.map(({ hour, active }) => <div aria-hidden="true" className={`h-14 border-r last:border-r-0 ${dark ? "border-slate-700" : "border-slate-200"} ${active ? dark ? "bg-sky-400" : "bg-blue-600" : hour < 7 || hour > 20 ? "bg-slate-800" : dark ? "bg-[#1e293b]" : "bg-[#f8fafc]"}`} key={hour} />)}</div><div className={`grid grid-cols-6 pt-1 text-[10px] font-semibold tracking-[0.12em] ${dark ? "text-slate-400" : "text-slate-500"}`}>{["00", "04", "08", "12", "16", "20"].map((hour) => <span key={hour}>{hour}</span>)}</div></div>;
}
...
export function App() {
  ...
  const accent = dark ? "hover:text-sky-400" : "hover:text-blue-600";
  const activeTheme = dark ? "bg-sky-400 text-slate-950" : "bg-blue-600 text-white";
  return <main className={`min-h-screen overflow-x-hidden px-4 py-6 sm:px-8 sm:py-10 ${dark ? "bg-[#111827] text-slate-100" : "bg-[#f6f7f8] text-slate-950"}`}><div className="mx-auto max-w-6xl">
